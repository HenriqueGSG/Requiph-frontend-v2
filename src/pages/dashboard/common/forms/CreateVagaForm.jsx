...
import { useCreateVaga } from "../sidebar/hooks/useCreateVaga";

const CreateVagaForm = ({ setShowModal }) => {
  const [role, setRole] = useState("");
  const [sector, setSector] = useState("-- Selecione um setor --");
  ...
  const preFormData = () =>
    Axios.get(
      "http://localhost:8000/api/dashboard/vaga/sector-company/list/"
    ).then((res) => res.data);
  const { data, isLoading } = useQuery(["sectorAndCompany"], () =>
    preFormData()
  );

  // const postVaga = async (role, sector, company, description, requirements) => {
  ...
  const { mutate } = useCreateVaga();
  const handleSubmit = (e) => {
    e.preventDefault();
    const vagaData = { role, sector, company, description, requirements };
    console.log({ role, sector, company, description, requirements });
    mutate(vagaData);
    setShowModal(false);
    ...
    <>
      <div className="block p-6 rounded-lg shadow-lg bg-white w-full">
        ...
        focus:text-gray-700 focus:bg-white focus:border-red-600 focus:outline-none"
              ...
              >
                {" "}
                <option value={sector} disabled hidden>
                  -- Selecione um setor --
                </option>
                {data.sector.map((sec) => (
